...
interface LogEntryWithBatch extends BatchLog {
  ...
  batchName: string;
  batchStrain: string;
  // Field status sudah ada di BatchLog dari database
}
...
function Main() {
  ...
  // 🔥 UPDATE - Export to CSV dengan status
  const exportToCSV = () => {
    const headers = [
            ...
            <Table>
              ...
              <Table.Tbody>
                {paginatedLogs.map((log, index) => {
                  // GUNakan SYSTEM CONFIG untuk menentukan status
                  const isTempNormal = log.temperature >= systemConfig.tempMin && log.temperature <= systemConfig.tempMax;
                  const isHumidityNormal = log.humidity >= systemConfig.humMin && log.humidity <= systemConfig.humMax;
                  const status = isTempNormal && isHumidityNormal ? 'Normal' : 'Warning';
                  
                  // Untuk styling warning
                  const isWarning = !isTempNormal || !isHumidityNormal;
                  ...
                  return (
                    <Table.Tr key={`${log.id}-${index}`}>
                      ...
                      {/* 🔥 TAMBAHAN - 3 Kolom Status dengan getStatusDisplay */}
                      <Table.Td>
                        <span className={clsx(
          ...
          {filteredLogs.length > 0 && (
            <div className="flex flex-col-reverse flex-wrap items-center p-5 flex-reverse gap-y-2 sm:flex-row">
              <Pagination className="flex-1 w-full mr-auto sm:w-auto">
                ...
                {paginationItems[0] > 1 && (
                  <>
                    <Pagination.Link onClick={() => handlePageChange(1)}>1</Pagination.Link>
                    {paginationItems[0] > 2 && <Pagination.Link className="cursor-default pointer-events-none">...</Pagination.Link>}
                  </>
                )}
                ...
                {paginationItems[paginationItems.length - 1] < totalPages && (
                  <>
                    {paginationItems[paginationItems.length - 1] < totalPages - 1 && (
                      <Pagination.Link className="cursor-default pointer-events-none">...</Pagination.Link>
                    )}
